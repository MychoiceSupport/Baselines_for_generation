...
class StandardScaler:
    ...
    def transform(self, data: torch.Tensor):
        ...
        raw_data = data.clone()
        raw_data[..., 0] = (data[..., 0] - self.mean[0]) / (self.std[0] + 1e-10)
...
class MAE_ViT(torch.nn.Module):
    def __init__(self, args,
        ...
        self.pos_dec = nn.Sequential(
            nn.Linear(self.emb_dim, self.emb_dim), nn.SiLU(), nn.Linear(self.emb_dim, 2)
        )

        self.encoder = MAE_Encoder(image_size, patch_size, emb_dim, encoder_layer, encoder_head, mask_ratio, self.emb_dim)
        self.decoder = MAE_Decoder(image_size, patch_size, emb_dim, decoder_layer, decoder_head, self.emb_dim)
        # self.time_enc = get_time_encoder('day', self.emb_dim, args.device)
        self.time_enc = nn.Embedding(288, self.emb_dim)
        self.time_lin = nn.Linear(2 * self.emb_dim, self.emb_dim)
    ...
    def get_time_fused_pos_emb(self, img, time_feats):
    ...
    def forward(self, img, time_feats):
        ...
        :return:
        """
        img = self.get_time_fused_pos_emb(img, time_feats)
        ...
        # predicted_img = self.pos_dec(predicted_img)
        predicted_img = self.pos_dec(predicted_img.transpose(-2,-1))
...
def evaluate_loader(args, logger, data_loader, model, optimizer, epoch, max_epoch, scaler, mode='train'):
    ...
    for gps_traj, grid_ids, edge_ids, ratios, time_feats, head in pbar:
        ...
        loss = loss_fn(rec_traj * mask[..., 0].unsqueeze(-1), gps_traj * mask[..., 0].unsqueeze(-1)) / args.mask_ratio
        ...
        if args.transform:
            ...
            rev_traj = rec_traj
        geo_loss = torch.mean(geodesic(rev_traj[torch.where(mask[..., 0] == 1)], raw_gps_traj[torch.where(mask[..., 0] == 1)]))
        all_geo_loss.append(geo_loss.item())
    ...
    train_geo_loss = np.mean(all_geo_loss)
    logger.info(f'The result for the training dataset is {train_loss} at epoch {epoch}, the recovered trajectory pos error is {train_geo_loss}!')
    return train_loss
...
def main(args, logger):
    ...
    # if os.path.exists('GPS_Pretrain_std.pt'):
    #     checkpoint = torch.load('GPS_Pretrain_std.pt')
    ...
    print(f"GPU Memory allocated: {torch.cuda.memory_allocated() / (1024 ** 2):.2f} MB")
    for epoch in range(0, args.training_epoch + 1):
        ...
        if test_loss < best_loss:
            ...
            if args.transform:
                torch.save(model.state_dict(), f'GPS_Pretrain_std_{args.dataset}_with_time_mask_ratio_{args.mask_ratio}_patch_size_{args.patch_size}.pt')
            else:
                torch.save(model.state_dict(), f'GPS_Pretrain_false_std_{args.dataset}_with_time_mask_ratio_{args.mask_ratio}_patch_size_{args.patch_size}.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device_ids', default=[2,2], help='all_device_ids')
    ...
    result_name = 'pretrain_gps'
    exp_dir = result_name
    get_args(args)
    root_dir = Path(__name__).resolve().parents[0]
    logger = Logger(
        __name__,
        log_path= root_dir / exp_dir / f"Pretrain_use_std_{args.dataset}.log",
        colorize=True,
    )
    setup_seed(42)
    device = args.device
