...
def load_rn_csv(node_path, edge_path, graph_path = 'road_newtork', is_directed=True, save = False):
    ...
    for i, env in enumerate(edges_data):
        ...
        edge_spatial_idx.insert(eid, envs)
...
def load_graph(file_path='road_graph', city_name='Porto', is_directed=True):
    ...
    with open(file_name, 'rb') as f:
        graph = pickle.load(f)
    graph.construct_all_region()
    # 从元数据加载 Rtree 路径并重建索引d
    return graph

def load_rn_csv_v2(node_path, edge_path, graph_path = 'road_newtork', is_directed=True, save = False):
    ...
    for i, env in enumerate(edges_data):
        ...
        if i == 0:
            print("查看数据:",coord_list[0][0], coord_list[0][1])
            print("查看数据2:", coords[0].lat, coords[0].lng)
        ...
        if i == 0:
            print("查看coords:", envs)
        # edge_spatial_idx.insert(eid, (envs[2], envs[0], envs[3], envs[1]), obj=eid)
        edge_bounds.append(envs)
        edge_index_id.append(eid)
    ...
    ####这一部分主要是看有没有重复的内容
    # edge_spatial_idx.close()
    if save == True:
        if not is_directed:
            Graph = UndirRoadNetwork(G, edge_bounds, edge_ids, edge_idx, max(df_nodes['ID']),(max(df_edges['fid'])))
        else:
            Graph = RoadNetwork(G, edge_bounds, edge_ids, edge_idx, max(df_nodes['ID']),(max(df_edges['fid'])))
    else:
        if not is_directed:
            Graph = UndirRoadNetwork(G, edge_bounds, edge_ids, edge_idx, max(df_nodes['ID']),(max(df_edges['fid'])))
            store_rn_graph_index(Graph)
        else:
            Graph = RoadNetwork(G, edge_bounds, edge_ids, edge_idx, max(df_nodes['ID']),(max(df_edges['fid'])))
            store_rn_graph_index(Graph)
    return Graph
...
if __name__ == '__main__':
    ...
    graph = load_rn_csv_v2('road_network/Porto/nodes.csv',
                'road_network/Porto/edges.csv',None, True, False)
    graph_new = load_graph()
    print(graph_new.edge_spatial_idx)