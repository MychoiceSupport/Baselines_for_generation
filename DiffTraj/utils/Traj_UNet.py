...
class Guide_UNet(nn.Module):
    def __init__(self, args, config, scaler):
        ...
        self.unet = Model(args, config, self.ch, self.attr_dim)
        self.kernel_size = self.args.kernel_size
        # self.guide_emb = Guide_Embedding(self.attr_dim, self.ch)
        # self.place_emb = Place_Embedding(self.attr_dim, self.ch)
        self.guide_emb = WideAndDeep(self.attr_dim)
        self.place_emb = WideAndDeep(self.attr_dim)
        graph = load_graph('road_graph', 'Porto', 'True')
        self.graph = dgl.from_networkx(graph.g, device=self.device)
        ...
        self.node_coords = torch.from_numpy(self.node_coords).float().to(self.device)
        self.node_ids = torch.from_numpy(np.array(self.node_id)).int().to(self.device)
        ...
        self.edge_neighbors = torch.from_numpy(self.edge_neighbors).int().to(self.device)
        self.road_length = torch.from_numpy(self.road_length).float().to(self.device)
        self.pos_emb = nn.Sequential(
            nn.Conv1d(2, self.embedding_dim, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2),
            nn.SiLU(),
            nn.Conv1d(self.embedding_dim, self.embedding_dim, self.kernel_size, 1, padding=(self.kernel_size - 1) // 2))

        self.attn_weight = nn.Conv1d(self.embedding_dim, 1, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2)
        if self.args.use_road_emb:
            self.id_emb = nn.Embedding(53826, self.args.emb_dim)
            self.ratio_emb = nn.Conv1d(1, self.args.emb_dim, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2)
            self.proj = nn.Conv1d(self.args.emb_dim * num_in, self.args.emb_dim, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2)
            self.id_argmax = nn.Sequential(
                nn.SiLU(),
                nn.Conv1d(self.args.emb_dim, 53826, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2)
            )
            self.ratio_dec = nn.Sequential(
                nn.SiLU(),
                nn.Conv1d(self.args.emb_dim, 1, self.kernel_size, stride=1, padding=(self.kernel_size - 1) // 2)
            )
        else:
            self.pos_dec = nn.Sequential(
                nn.Conv1d(self.embedding_dim, self.embedding_dim, self.kernel_size, 1,
    ...
    def get_road_emb(self, edge_ids):
        ...
        id_embedding = self.id_emb(edge_ids).reshape(batch * step, -1)
        if self.args.transform:
            edge_feats = self.scaler.transform(self.node_coords[edge_ids.long()]).float()
        else:
            edge_feats = self.node_coords[edge_ids.long()].float()
        # print("查看其中的形状:",self.node_mask[edge_ids.long()].shape)
        # L = self.node_mask.shape[-2]
        ## N * T * L * 2
        edge_feats = edge_feats.transpose(-1, -2).reshape(batch * step, 2, -1)
        ## N * T, 2, L
        ...
        attn_scores = self.attn_weight(road_emb).squeeze().transpose(-2,-1)
        attn_weights = F.softmax(attn_scores,dim=-1)
        weighted_feats = attn_weights.transpose(-2,-1).unsqueeze(-1) * road_emb.transpose(-1,-2)
        road_emb = weighted_feats.sum(dim=-2)
        # road_emb = torch.sum(road_emb, dim=-2)  # N * T * L * d -> N * T * L

        return road_emb + id_embedding
